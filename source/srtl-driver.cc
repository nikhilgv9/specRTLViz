...
QString pToString(const yy::position pos){
    QString s;
    if (pos.filename)
        s += QString(*pos.filename->c_str()) + ':';
    s += QString::number(pos.line) + '.' + QString::number(pos.column);
    return s;
}

QString lToString(const yy::location& l){
    QString s;
    yy::position last = l.end - 1;
    s += pToString(l.begin);
    if (last.filename
    && (!l.begin.filename
        || *l.begin.filename != *last.filename))
      s += '-' + pToString(last);
    else if (l.begin.line != last.line)
      s += '-' + QString::number(last.line)  + '.' + QString::number(last.column);
    else if (l.begin.column != last.column)
      s += '-' + QString::number(last.column);
    return s;
}

void srtl_driver::error (const yy::location& l, const std::string& m) {
    srtl_driver::messages=lToString(l)+":"+QString(m.c_str());
    std::cerr << l << ": " << m << std::endl;
}

void srtl_driver::error (const std::string& m) {
    srtl_driver::messages=QString(m.c_str());
    std::cerr << m << std::endl;
}
